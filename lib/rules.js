...
rules.push({
	...
	desc: "Try to inline font files. Also make them compressed and cacheable",
	...
	check: function($, cssContent, reporter) {		
		...
		fontFaceMatches.forEach(function(match) {		
			...
			match = match.replace(/local\([^\)]*\)[\s]*[,]?/gim, '');			
			var fontUrl;
			while(fontUrl = urlPattern.exec(match)) {
				reporter.warn("WARNING: Possible SPOF attack due to @font-face style in external stylesheet", fontUrl[1], "NA", rule);	
			}
		});		
	}
});
...
rules.push({
	...
	desc: "Make sure the external fonts are compressed, cached and small in size",
	...
	check: function($, cssContent, reporter) {		
		...
		fontFaceMatches.forEach(function(match) {
			...
			var fontUrl;
			while(fontUrl = urlPattern.exec(match)) {
				reporter.warn("WARNING: Possible SPOF attack due to inline @font-face style", fontUrl[1], "NA", rule);	
			}
		});		
	}
});
