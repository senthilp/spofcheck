		...
		for(i = 0, l = scripts.length; i < l; i++) {
			...
			if(src && is3P(src)) {
				if(!script.defer || !script.async) {
					reporter.error("ERROR: Possible SPOF attack due to 3rd party script - " + src + " Fix: " + rule.desc, 
									src, getSeverity(src), rule);			
				}
			}
		}		
	}
...
rules.push({
	...
	check: function($, cssContent, reporter) {
		...
		for(i = 0, l = scripts.length; i < l; i++) {
			...
			src = script.src;
			if(src && !is3P(src)) {
				severity = getSeverity(src);				
				if(severity > 50 && (!script.defer || !script.async)) {					
					reporter.error("WARNING: Possible SPOF attack due to script " + src + " Fix: " + rule.desc, 
									src, severity, rule);			
				}
			}
		}		
